...
1. **Single-write, multi-read.** The compute domain writes scene data once. All consumers read from the same physical storage, not from independent copies. This eliminates redundant VRAM traffic.

2. **Coherent snapshot.** The hub holds a coherent snapshot of the current frame's intermediate data. Consumers read the same version, avoiding race conditions between, for example, the photonic renderer and the denoiser.

3. **Close to compute.** The hub sits on the same package as the compute chiplets — not in external HBM. This reduces read latency from ~100 ns (HBM) to ~10 ns (on-package SRAM or embedded DRAM).

4. **Protocol-agnostic.** The hub does not interpret the data — it stores opaque blocks tagged by type (G-buffer, motion, variance) and serves them to any consumer that requests a block by type and tile coordinates.
...
## 33.1 Use Cases: Optical Splitter Applications

The optical broadcast property described in Section 9 has two immediate practical applications where a passive optical splitter replaces active electronic duplication. These use cases do not require the full photonic rendering pipeline — they apply as soon as the optical image representation exists at the output of the photonic chiplet.

### 33.1.1 Game Streaming and Capture

In a conventional system, capturing a rendered frame for live streaming requires multiple redundant data paths:

```
GPU render → VRAM write
  → Display controller reads frame → HDMI/DP → monitor
  → NVENC reads same frame from VRAM → H.264/AV1 encode → streaming
  → [optional] Capture card re-reads HDMI → re-encodes → streaming
```

The same pixel data is read from VRAM two to three times. NVENC occupies GPU silicon area and memory bandwidth. A capture card is a separate device with its own input buffer, scaler, and encoder — adding 5–20 ms of latency.

In the proposed architecture:

```
Photonic chiplet → optical image (single source)
  → optical splitter (passive)
     ├── waveguide → optical display (zero latency, zero compute)
     └── waveguide → O/E converter → hardware encoder → streaming
```

The rendered frame is produced once as an optical signal. A passive splitter creates a physical copy — no VRAM re-read, no GPU compute overhead, no capture card. The encoder receives an optical tap of the same signal that drives the display, with negligible added latency.

| Parameter | Conventional | Optical splitter |
|-----------|-------------|-----------------|
| VRAM reads per frame for capture | 1–2 additional | 0 |
| GPU compute overhead for capture | NVENC: 2–5% GPU time | 0 |
| Capture card latency | 5–20 ms | ~0 (passive tap) |
| Extra hardware | Capture card + HDMI splitter | Photonic splitter (passive, on-package) |
| VRAM bandwidth consumed by capture at 4K120 | ~3.7 GB/s | 0 |

This use case is immediately understandable to a non-specialist audience: the streamer's GPU no longer does double duty, and the capture card disappears as a device.

### 33.1.2 Display Cloning

When the same image must appear on multiple displays simultaneously — exhibition walls, arcade cabinets, simulator cockpits, digital signage, broadcast control rooms — conventional architectures duplicate the framebuffer for each output:

```
VRAM → Display controller 1 → Display 1
VRAM → Display controller 2 → Display 2
VRAM → Display controller 3 → Display 3
```

Each display controller independently reads the same frame from VRAM. At 4K120, each read consumes ~3.7 GB/s. Three cloned displays consume ~11 GB/s of VRAM bandwidth for identical data.

With optical splitting:

```
Photonic chiplet → optical image
  → splitter → waveguide → Display 1
  → splitter → waveguide → Display 2
  → splitter → waveguide → Display 3
```

A single optical signal is split passively. Each display receives an optical copy. VRAM is read once, not N times. The splitter is a passive component — no power, no compute, no serialization per output.

This applies **only to display cloning** (same image on all outputs). Extended desktop modes, where each display shows different content, require separate optical paths from the photonic renderer — the splitter provides no benefit in that case.

### 33.1.3 What These Use Cases Do Not Cover

Optical splitting is fundamentally a broadcast mechanism. It does not help when:

- **Extended displays** need different content per screen — each requires a separate render.
- **VR** needs two different eye views — left and right eye receive different perspectives, so a simple optical copy is insufficient. The photonic renderer would need to produce two separate optical fields, one per eye, rather than splitting one.
- **Compositing** needs to combine multiple sources — a splitter distributes one source to many, not many sources into one.

The value of the optical splitter is therefore narrow but concrete: **it eliminates redundant data movement for identical copies of the same frame**, which today consumes VRAM bandwidth, GPU compute, and external hardware.
...
## 46. Broader Application Domains

The core principle — "do not convert light back to electricity without necessity" — is not specific to gaming displays. If the base assumptions hold (photonic rendering + optical signal preservation through the pipeline), the same logic transfers to any system with high-bandwidth video data, latency-critical visual feedback, or simultaneous distribution of the same visual stream to multiple recipients.

The following domains inherit the architecture's benefits with different priority weights. **None of them resolves the open engineering questions of Section 31 — they demonstrate that the value of resolving those questions extends beyond a single market.**

### 46.1 Healthcare — Real-Time Visualization

Endoscopy, surgical robotics, intraoperative ultrasound: the surgeon makes decisions based on images with single-digit-millisecond latency budgets. The current bottleneck mirrors the gaming case — sensor → processing → surgeon's display, with the path remaining largely electronic.

Optical broadcast is particularly relevant for simultaneous display of the same image on multiple monitors in the operating room (surgeon, assistants, teaching monitor) — a direct extension of display cloning (§33.1.2), but with far higher cost of error from latency.

**Inherited open questions with different weights:** Precision and reliability become safety-critical (§29.3, §31.2). A single point of failure in the optical path is a clinical risk, not a visual annoyance. Regulatory certification (FDA, CE) adds a layer absent from consumer gaming.

### 46.2 AR/VR and Wearable Displays

This is the most direct application — TriLite (§44, [3]) already builds laser beam scanning for AR. Weight, power, and latency constraints are stricter than for desktop monitors, making the potential gain from eliminating intermediate electronic stages even more significant.

However, as noted in §33.1.3, VR (two different images per eye) does not benefit from simple optical splitting. Separate rendering per eye is required, which complicates rather than simplifies this subdomain.

**Inherited open questions with different weights:** Addressing rate (§26) and power budget (§20) are more constrained. Form factor demands integration density beyond desktop packaging.

### 46.3 Avionics and Automotive HUD

Head-up displays in aircraft cockpits and vehicles are already partially optical (projection onto glass). The industry standard remains electronic up to the final projection stage. The proposed architecture potentially eliminates part of this path.

Weight and power matter more in aviation than in any desktop context — every gram has a certified cost. Latency in HUDs is safety-critical under certification standards (DO-178C, AS9100).

**Inherited open questions with different weights:** Reliability and certification dominate. The optical path must meet fault-tolerance standards that consumer hardware never encounters.

### 46.4 Control Centers, Dispatch, Video Walls

Direct scaling of display cloning (§33.1.2): airport operations centers, mission control, broadcast studios — where the same stream is physically duplicated to tens or hundreds of screens. The larger the number of clone screens, the greater the saving from passive optical splitting versus N independent electronic copies.

**Inherited open questions with different weights:** At large scale (hundreds of displays), cascaded passive splitters require optical amplification — the splitter is passive, but the distribution system is not. This is an engineering constraint not present in the 2–3 display gaming case.

### 46.5 Machine Vision and Industrial Inspection

The reverse direction: not "render → human" but "sensor → photonic processing → decision." ACCEL (§44, [1]) and OPCA (§44, [6]) already operate in this space — photonic computation for image classification on conveyor inspection and robotic vision.

The proposed architecture is potentially applicable not only for rendering to a human viewer but for sensor-to-decision pipelines that bypass electronic conversion at the input stage.

**Inherited open questions with different weights:** Precision (§29.3) is task-dependent — 4–8 bits may suffice for defect classification but not for metrology. The input interface question (§5, TASK-01) applies symmetrically: optical sensor → photonic processor vs. optical sensor → E/O → electronic processor.

### 46.6 Satellite and Long-Distance Image Transmission

If data is already optical and transmitted over fiber (satellite and backbone communications are already optical), preserving the signal as optical "from render to transmission" logically interfaces with existing optical communication infrastructure — fewer conversions at the compute-to-transport boundary.

**Inherited open questions with different weights:** The interface must bridge to communication standards (DWDM, ITU-T grid) rather than to a local display. This is a protocol-bridging problem, not a rendering problem.

### 46.7 Strategic Significance

The expansion of application domains does not resolve any of the open engineering questions identified in Section 31. Each domain inherits the same questions with different priority weights:

| Domain | Precision | Latency | Reliability | Scale (broadcast) | Power |
|--------|-----------|---------|-------------|-------------------|-------|
| Gaming (baseline) | Medium | Medium | Low | 2–3 displays | Medium |
| Healthcare | **Critical** | **Critical** | **Critical** | 3–5 monitors | Medium |
| AR/VR | Medium | **Critical** | Medium | 1–2 displays | **Critical** |
| Avionics / HUD | Medium | **Critical** | **Critical** | 1–2 displays | **Critical** |
| Control centers | Medium | Medium | High | **10s–100s** | Medium |
| Machine vision | Task-dependent | High | High | N/A | Low |
| Satellite | Low | Low | High | N/A | Low |

The strategic value of this analysis is not technical proof but market evidence: **if the engineering problems are resolved, the architecture's value is not limited to one market.** This strengthens the argument for research investment (§45) by demonstrating that the payoff is multi-domain, not single-market.
